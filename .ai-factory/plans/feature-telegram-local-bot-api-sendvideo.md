<!-- handoff:task:f29132d3-e6e2-45b4-a8b5-003981e2fb17 -->
...
- [ ] Branch handling: active implementation branch is `feature/telegram-local-bot-api-sendvideo`. Do not repeat branch creation during implementation; preserve the existing uncommitted plan-file move/rename state unless the user explicitly asks to clean it up.
...
- [x] Task 1: Add Telegram upload mode configuration helpers.
  ...
  - [ ] Expected behavior: `loadServerConfig` still parses `telegram.apiRoot` from `TELEGRAM_API_ROOT`; validate/normalize it as a Bot API root URL, strip trailing slashes, reject or fail fast if it includes a `/bot<TOKEN>` suffix, and treat an unset value or explicit official cloud API root as cloud mode. Add adapter-safe helpers/constants such as cloud limit `50 MB`, local limit `2000 MB`, `isLocalBotApiMode`, `telegramUploadLimitBytes`, and a single upload-policy object that server composition can pass into both `TelegramResultSender` and metadata/menu display adapters. grammY still receives root URL only, with no `/bot<TOKEN>` suffix. `MAX_FILE_SIZE_MB` remains server-side download policy and does not mean Telegram can send the file.
...
- [x] Task 2: Extend `TelegramResultSender` to choose `sendVideo` for actual `.mp4` outputs.
  ...
  - [ ] Expected behavior: before any upload, call `fs.stat` on `result.filePath`; reject over-limit files with Russian user copy before uploading. Detect the actual output extension with `path.extname(result.filePath).toLowerCase()`, not with requested format or unsanitized display text. If the actual output extension is `.mp4`, call `sendVideo(chatId, file, { caption, supports_streaming: true })`; otherwise call `sendDocument`. WEBM and other non-MP4 files must not use `sendVideo`. Use `InputFile(result.filePath)` as the safe fallback for both cloud and local modes. Do not add ffprobe as a required gate in V1. When `TelegramResultSender` has already sent a user-facing upload/size/send-failure message and must rethrow for job state accuracy, it must expose a concrete typed/sentinel failure, for example a `TelegramResultAlreadyNotifiedError` plus `isTelegramResultAlreadyNotifiedError(error)`, so the server `onJobFailed` path can suppress the generic `sendFailure` message; users must not receive both a specific upload failure and a generic failure for the same send attempt.
  - [ ] Files: `src/adapters/telegram/result-sender.ts`, `src/adapters/telegram/copy.ts`, `src/adapters/telegram/result-sender.test.ts`, `src/server/index.ts`, optionally `src/adapters/telegram/result-errors.ts`.
...
- [ ] Task 9: Align metadata dispatch and display policy with upload limits.
  ...
  - [ ] Dependencies: tasks 1, 4, 5, 6, and 7.
